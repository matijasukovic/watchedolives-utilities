...
parser = argparse.ArgumentParser(description='For a given YOLO subset, extracts a list of image filenames and saves as annotations.txt in the labels directory. This is used when uploading a YOLO dataset to CVAT, in order to align with the YOLO format CVAT expects.')
